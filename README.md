...
- **File Organization & Auto-Numbering**: Rename audio files directly from tags using customizable syntax templates (e.g. `%tracknumber% - %artist% - %title%`), preserving original file extensions (`.flac` or `.mp3`), or renumber tracks sequentially (with optional zero-padding `01, 02...` configurable in Preferences).
