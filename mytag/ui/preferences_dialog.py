...
class PreferencesDialog(Adw.Dialog):
    ...
    def __init__(self):
        ...
        page.add(self._build_organization_group())
    ...
    def _build_organization_group(self) -> Adw.PreferencesGroup:
        group = Adw.PreferencesGroup(title=i18n.t("prefs.organization_group"))

        expander = Adw.ExpanderRow(
            title=i18n.t("prefs.naming_options"),
            subtitle=i18n.t("prefs.naming_options_subtitle"),
        )
        ...
        switch_row = Adw.SwitchRow(
            title=i18n.t("prefs.zero_padding"),
            subtitle=i18n.t("prefs.zero_padding_subtitle"),
        )
        switch_row.set_active(self._pending_zero_padding)

        def on_switch_changed(row: Adw.SwitchRow, _pspec) -> None:
            self._pending_zero_padding = row.get_active()

        switch_row.connect("notify::active", on_switch_changed)
        expander.add_row(switch_row)

        pattern_row = Adw.EntryRow(title=i18n.t("prefs.default_rename_pattern"))
        pattern_row.set_text(self._pending_rename_pattern)

        def on_pattern_changed(row: Adw.EntryRow) -> None:
            self._pending_rename_pattern = row.get_text().strip() or "%tracknumber% - %artist% - %title%"

        pattern_row.connect("changed", on_pattern_changed)
